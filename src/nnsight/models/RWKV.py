...
class RWKV_5(AbstractModel):
    ...
    def _load_meta(self, repoid_or_path, *args, device=None, **kwargs) -> PreTrainedModel:
        ...
        self.config = RwkvConfig.from_pretrained("RWKV/rwkv-4-169m-pile")
        model = RwkvForCausalLM(self.config)
        return model.to('meta')
    
    def _load_local(self, repoid_or_path, *args, **kwargs) -> PreTrainedModel:
        model = RwkvForCausalLM.from_pretrained(repoid_or_path).cuda()
